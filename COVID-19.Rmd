...
erstellt und findet sich unter https://github.com/stamats/COVID-19.
...
```{r, fig.width=7, fig.height=5, fig.cap="Abb. 1: Bevölkerung in Deutschland 2019 aufgeteilt in Altersgruppen."}
library(ggplot2)
library(ggsci)
...
library(MKinfer)
p.0.20 <- (0+1)/(416+549)
p.0.20.CI <- binomCI(x = 1, n = 416+549)$conf.int
p.20.40 <- (7+18)/(3619+7600)
p.20.40.CI <- binomCI(x = 7+18, n = 3619+7600)$conf.int
p.40.50 <- 38/8571
p.40.50.CI <- binomCI(x = 38, n = 8571)$conf.int
p.50.60 <- 130/10008
p.50.60.CI <- binomCI(x = 130, n = 10008)$conf.int
p.60.70 <- 309/8583
p.60.70.CI <- binomCI(x = 309, n = 8583)$conf.int
p.70.80 <- 312/3918
p.70.80.CI <- binomCI(x = 312, n = 3918)$conf.int
p.80plus <- 208/1408
p.80plus.CI <- binomCI(x = 208, n = 1408)$conf.int
## kumuliert
...
p.kum.CI <- binomCI(x = 0+1+7+18+38+130+309+312+208, 
                    n = 416+549+3619+7600+8571+10008+8583+3918+1408)$conf.int
...
2.29 % (95%-KI: 2.16 - 2.43 %). Dies liegt etwas über der 
...
#  geom_ribbon(aes(ymin = Tote.low, ymax = Tote.upp, fill = Altersgruppe)) +
  ...
  annotate("text", x = rep(15, 8)-(c(3.5,3.5,3.5,3.5,4,4.5,4.5,4.5)+2), 
           y = DF$Tote[DF$Durchseuchung == 15]*c(1,1.06,0.94,1,1,1,1,1),
  ...
  annotate("text", x = rep(85, 8)+c(4,4,4,4,4.5,4.5,4.5,5)+2, 
           y = DF$Tote[DF$Durchseuchung == 85]*c(1,1.06,0.94,1,1,1,1,1),
...
04.04.2020 [@Worldometers2020]. Es gab bis zu diesem Datum 96092 bekannte Fälle 
von COVID-19 in Deutschland. Davon sind 26400 bereits wieder gesund und 1444 
verstorben, was 27844 abgeschlossenen und 68248 aktiven Fällen entspricht. 
...
ggplot(DF, aes(x = Altersgruppe, y = Tote, fill = Altersgruppe)) +
  ...
  geom_errorbar(aes(ymin = Tote.low, ymax = Tote.upp),      
                color = "black", width = 0.15,
                position = position_dodge(.5)) +
  annotate("text", x = 1:8, y = DF$Tote+c(40, 30, 30, 30, 30, 40, 60, 150), 
           label = DF$Tote) +
  xlab("Altersgruppe") + ylab("Tote")
```

Demnach sollten sich unter den abgeschlossenen Fällen `r round(Tote.kum)` Tote 
(95%-KI: 612 - 824) befinden. Ein Grund für den Unterschied 
...
Dies führt auf eine geschätzte Durchseuchung von ca.\ 689 pro Million 
(95%-KI: 587 - 789). Sollte 
dies zutreffen, so ergeben sich nicht 27844, sondern bereits 57309 abgeschlossene 
COVID-19 Fälle (95%-KI: 48802 - 65675) in Deutschland. Dies würde bedeuten, dass 
bisher ca.\ die Hälfte (95%-KI: 42.4 - 57.1 %) der COVID-19 Fälle in Deutschland 
